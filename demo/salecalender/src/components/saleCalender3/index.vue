  ...
  <div class="sale-calender-content" :class="!activeItem.isLessThanToday?'isBottom':''">
    <div class="content">
      ...
      <SaleCalender
        ...
        @renderYearAndMonth="renderYearAndMonth">
      </SaleCalender>
      <div class="line"></div>
      <!-- 筛选项 -->
    ...
    <div class="loading-box" v-show="loading"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    queryDateList(beginDate, endDate, checkedDay, type, weekDay){
      ...
      let me= this;
      me.beforeRenderData(checkedDay, type, weekDay);
      me.loading = true;
      ...
      .then(function (res) {
        ...
        me.$refs.saleCalender.addSalesData(res.data.data);
      },e=>{
        me.loading = false;
      });
    },
        // 解决接口没出来前页面不显示日历问题
    beforeRenderData(checkedDay, type, weekDay){
      ...
      if(type == 'changeMonth'){
        this.$refs.saleCalender.reDrawMonth(checkedDay, weekDay);
      }else if(type == 'changeWeek'){
        this.$refs.saleCalender.reDrawWeek(checkedDay, weekDay);
      }
    },
    ...
    overscroll(el) {
      ...
      el.addEventListener('touchend', function(e) {
        if(distanceY > me.slideDistance){
          me.$refs.saleCalender.doDown();
        }else if(distanceY < -me.slideDistance){
          me.$refs.saleCalender.doUp();
        }
      });
    },
    moveFn(evt){
...
.content .m-header .arrowleft:after, .content .m-header .arrowleft:before {
...
.content .m-header .arrowleft:before {
  border-right-color: #3B83FF;
  left: -0.16rem;
}

.content .m-header .arrowright:after, .content .m-header .arrowright:before {
  border: 0.14rem solid transparent;
...
.content .m-header .arrowright:before {
